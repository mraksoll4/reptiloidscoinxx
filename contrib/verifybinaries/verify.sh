#!/usr/bin/env bash
# Copyright (c) 2016 The ReptiloidsCoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   reptiloidscoincore.org and reptiloidscoin.org and compares them.
...
WORKINGDIR="/tmp/reptiloidscoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://reptiloidscoincore.org"
HOST2="https://reptiloidscoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="reptiloidscoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'reptiloidscoin-',
...
if [ $? -ne 0 ]; then
   echo "reptiloidscoin.org failed to provide signature file, but reptiloidscoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "reptiloidscoin.org and reptiloidscoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the ReptiloidsCoin Core binary release signing key installed?"
   fi
