...
const char * const REPTILOIDSCOIN_CONF_FILENAME = "reptiloidscoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "reptiloidscoind -foo=bar
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", REPTILOIDSCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure ReptiloidsCoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("ReptiloidsCoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The ReptiloidsCoin Core developers";
    }
