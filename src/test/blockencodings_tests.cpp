...
static CBlock BuildBlockTestCase() {
    ...
    assert(!mutated);
    while (!CheckProofOfWork(phashdb->GetHash(block), block.nBits, Params().GetConsensus())) ++block.nNonce;
    return block;
}
...
BOOST_AUTO_TEST_CASE(SimpleRoundTripTest)
{
    ...
    // Do a simple ShortTxIDs RT
    {
        ...
        BOOST_CHECK_EQUAL(phashdb->GetHash(block).ToString(), block3.GetHash().ToString());
...
BOOST_AUTO_TEST_CASE(SufficientPreforwardRTTest)
{
    ...
    // Test with pre-forwarding coinbase + tx 2 with tx 1 in mempool
    {
        ...
        BOOST_CHECK_EQUAL(phashdb->GetHash(block).ToString(), block2.GetHash().ToString());
        bool mutated;
