...
const std::string ReptiloidsCoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
#ifdef ENABLE_WALLET
    if(enableWallet) {
        connect(walletFrame, &WalletFrame::requestedSyncWarningInfo, this, &ReptiloidsCoinGUI::showModalOverlay);
        connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &ReptiloidsCoinGUI::showModalOverlay);
        connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &ReptiloidsCoinGUI::showModalOverlay);
    }
#endif
...
void ReptiloidsCoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    ...
    // Append title to "ReptiloidsCoin - "
    if (!msgType.isEmpty())
        strTitle += " - " + msgType;
...
void ReptiloidsCoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    ...
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(ReptiloidsCoinUnits::formatWithUnit(unit, amount, true));
    ...
    menu(nullptr)
{
    ...
    QList<ReptiloidsCoinUnits::Unit> units = ReptiloidsCoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const ReptiloidsCoinUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(ReptiloidsCoinUnits::longName(unit)));
    }
...
void UnitDisplayStatusBarControl::createContextMenu()
{
    ...
    {
        QAction *menuAction = new QAction(QString(ReptiloidsCoinUnits::longName(u)), this);
...
{
    setText(ReptiloidsCoinUnits::longName(newUnits));
}
