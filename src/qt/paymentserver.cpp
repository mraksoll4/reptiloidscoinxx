...
const int REPTILOIDSCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString REPTILOIDSCOIN_IPC_PREFIX("reptiloidscoin:");
#ifdef ENABLE_BIP70
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the reptiloidscoin: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(REPTILOIDSCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // reptiloidscoin: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseReptiloidsCoinURI(arg, &r) && !r.address.isEmpty())
            {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(REPTILOIDSCOIN_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(REPTILOIDSCOIN_IPC_CONNECT_TIMEOUT);
...
#endif
{
    ...
    // on Mac: sent when you click reptiloidscoin: links
...
//
// OSX-specific way of handling reptiloidscoin: URIs and PaymentRequest mime types.
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    else if (s.startsWith(REPTILOIDSCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // reptiloidscoin: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseReptiloidsCoinURI(s, &recipient))
            {
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in reptiloidscoin: URIs
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom reptiloidscoin addresses are not supported
        ...
        // ReptiloidsCoin amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (IsDust(txOut, optionsModel->node().getDustRelayFee())) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(ReptiloidsCoinUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
